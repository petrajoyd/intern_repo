...
Authors = CHENG-FENG HUNG, CHI-HENG TSENG, AND SHIN-MING CHENG </br>
Year = 2025 (17 March) </br>
@paper_2

## Abstract
As 5G network evolve, the O-RAN introduces greater opennes and flexibility, but this also expands the attack surface, especially around the near-RT RIC. In particular, threats targeting xApps and the E2 interface can directly affect RAN control decisions during runtime. </br>
This paper proposes an anomaly-based traffic detection mechanism to protect the near-RT RIC from both xAPPA-related threats and E2 interface attacks. Instead of relying on predefined signatures, the approach monitors traffic behaviour and identifies deviations from normal patterns. The detector is designed to operate within near-RT constraints and can be integrated into the RIC without disrupting existing control workflows. </br>
Experimental results show that the proposed anomaly traffic detector is effective in identifying malicious behaviour while maintaining acceptable processing latency. The findings indicate that anomaly-based detection is a practical and scalable solution for improving runtime security in O-RAN near-RT RIC environments.

## Background (Bonus)
The paper starts from the idea that O-RAN fundamentally changes how security should be handled in the RAN. By disaggregating the network and opening interfaces like E2, O-RAN enables flexibility and innovation, but at the same time removes the traditional trust boundaries found in monolithc RAN systems. </br>
In the O-RAN architecture, the near-RT RIC becomes a critical control point. It hosts xApps that directly influence radio resource management, scheduling, and mobility decisions. Because these decisions are made in near real time, anu malicious behaviour affecting the near-RT RIC can immediately impact network performance and user experience. </br>
The paper highlights two main sources of risk in this encironment.
1. The xApps themselves. Since xApps can be developed by third parties and deployed dynamically, they may be buggy, misconfigured, or malicious. Once running, a compromised xApp has legitimate access to control interfaces, making it difficult to distinguish between normal and malicious behaviour using static checks alone.
2. The E2 interface is identified as a major attack surface. The E2 interface carries control messages and telemetry between the RIC and E2 nodes. Even if messages are protocol-compliant, attackers can still exploit traffic patterns, message rates, or payload characteristics to degrade performance or influence RIC behaviour.

The paper says that traditional security mechanisms are not sufficent in this context. Authentication, authorization, and secure onboarding only verify component at boarding (deployment) time. They do not address runtime threats, especially those that manifest as abnormal traffic behaviour rather than explicit protocol violations. </br>

Because of the issues mentioned above, the paper proposed the need for behaviour-based (anomaly-based) detection. Instead of relying on predefined attack signatures, anomaly detection models learn what normal traffic looks like in the near-RT RIC environment and flag deviations during operation. This approach is better suited for detecting unknown attacks, misbehaving xApps, and subtle E2 interface abuse.


## System Model & Architecture
The system is based on the standard O-RAN architecture, where the near-RT RIC sits between the SMO and the RAN nodes (O-DU / O-CU). The near-RT RIC hosts xApps and communicates with E2 nodes through the E2 interface, which carries both control messages and telemetry.

<p align="center">
    <img src="../../NTN/notes-png/overview of O-RAN architecture with anomaly traffic detector.png" alt="Overview Of O-RAN Architecture With Anomaly Traffic Detector Framework"/>
    <br />
    <em>Overview Of O-RAN Architecture With Anomaly Traffic Detector Framework</em>
</p>

In this paper, an anomaly traffic detector is integrated into the near-RT RIC. The key idea is that the detector observes traffic behaviour at runtime, rather than inspecting message contents or relying on predefined attack signatures.

The detector monitors:
- Traffic generated by xApps inside the near-RT RIC
- Traffic coming from E2 nodes via the E2 interface

The rest of the O-RAN system remains unchanged. The detector is added as a monitoring and detection component, not as a replacement for existing RIC functions. This makes the approach compatible with current O-RAN deployments.

The anomaly traffic detector is logically placed inside the near-RT RIC, where it can observe both:
- Internal traffic between xApps and RIC services
- External traffic exchanged over the E2 interface

This position allows the detector to identify abnormal traffic patterns that could indicate:
- Misbehaving or malicious xApps
- Attacks targeting the RIC via the E2 interface

Importantly, the detector does not interfere directly with control decisions. It operates as an observer and raises alerts when anomalies are detected.

<p align="center">
    <img src="../../NTN/notes-png/Anomaly-traffic-detector-architecture.png" alt="Anomaly Traffic Detector Architecture" />
    <br />
    <em>Anomaly Traffic Detector Architecture</em>
</p>

The detector architecture consists of three main stages:
- Traffic Collection
    - Traffic data is collected from the near-RT RIC environment.
    - This includes traffic metrics such as packet counts, rates, and timing characteristics.
    - No deep packet inspection or payload analysis is performed.

- Feature Extraction
    - Raw traffic data is processed into feature vectors that represent traffic behaviour over time.
    - Features capture patterns such as traffic intensity and temporal variation.
    - These features are used as inputs to the anomaly detection model.

- Anomaly Detection Engine
    - A machine-learning-based anomaly detection model is applied to the extracted features.
    - The model learns what normal traffic behaviour looks like during training.
    - During runtime, deviations from normal patterns are flagged as anomalies.

When an anomaly is detected, the system generates an alert that can be used for further action, such as investigation or mitigation by higher-level security mechanisms.

## Security Assumptions
- The near-RT RIC platform is assumed to be trusted (deployment assumptions)
- Communication channels are assumed to be protected at the transport level
  - The paper assumes that the attacks happen despite protocol-level security, through normal abnormal behaviour
- xApps are not fully trusted at runtime
- E2 nodes are treated as potentially untrusted
  - O-DUs / O-CUs may be compromised or misbehave
  - Traffic coming over the E2 interface cannot be blindly trusted.
- The attacker is assumed to be active during runtime, not before deployment  

## Threat Model / Attack Scenario
The paper considers attacks that target the runtime behaviour of the near-RT RIC, specifically through traffic anomalies rather than protocol violations.

### A. xApp-based Attacks
- A malicious or compromised xApp generates abnormal traffic patterns toward the near-RT RIC. This includes:
  - Excessive control requests
  - Unexpected message rates
  - Irregular communication behaviour compared to normal xApps
- Since the xApp is authorized,traditional access control does not stop this attack
- The goal is to degrade RIC performance or influence RAN control decisions.

### B. E2 Interface Attacks
- Attacks originate from the E2 interface between the near-RT RIC and E2 nodes. 
- Messages are protocol-compliant, but their traffic characteristics are abnormal. This includes
  - Flooding the RIC with E2 messages
  - Abnormal reporting frequency
  - Irregular traffic bursts
- These attacks can overload the RIC or interfere with normal control-loop operation   

### C. Key Point Of The Threat Model
- The attacker does not need to break authentication or encryption
- Attacks rely on abusing legitimate interfaces and normal-looking messages
- This makes signature-based or rule-based detection insufficient on its own

### Why The Paper's Proposal Fix Fits?
- Normal near-RT RIC traffic has learnable patterns.
- Attacks cause deviations in traffic behaviour
- Detecting anomalies allows the system to indentify both known and unknown attacks at runtime.

## Methodology
The methodology starts by looking at how normal communication happens over the E2 interface. In a standard setup, xApps inside the near-RT RIC communicate with E2 nodes using E2 messages for control and telemetry exchange. This process follows a well-defined message sequence and timing behaviour.

<p align="center">
    <img src="../../NTN/notes-png/The E2 Communication process between xApp And E2 node, and the possible attack scenarios INvolved.png" alt="The E2 Communication Process Between xApp and E2 Node, And The Possible Attack Scenarios Involved" />
    <br />
    <em>The E2 Communication Process Between xApp and E2 Node, And The Possible Attack Scenarios Involved</em>
</p>

The paper points out that most attacks do not break the protocol itself. Instead, they abuse the communication process by sending messages that are:
- Out of sequence
- Sent at abnormal rates
- Contain malformed or non-compliant fields

The figure highlights where attacks can be injected along this communication path, either from a malicious xApp or through the E2 interface. The key idea is that even when messages are syntactically valid, their behaviour over time may not be normal.

This observation is what motivates a behaviour-based detection approach instead of pure protocol validation.

</br>

To model normal and abnormal behaviour, the paper introduces a state machine-based analysis module. The idea is to represent the E2 communication process as a sequence of valid states and transitions.

<p align="center">
    <img src="../../NTN/notes-png/The-state-machine-analysis-module.png" alt="The State Machine Analysis Module" />
    <br />
    <em>The State Machine Analysis Module</em>
</p>

Each state corresponds to a specific phase of communication, such as:
- Connection setup
- Message exchange
- Control or reporting phases

Under normal operation, traffic follows predictable state transitions. When an attacker injects malicious traffic, these transitions may occur:
- Too early
- Too frequently
- In an invalid order

The anomaly traffic detector monitors traffic and checks whether observed behaviour aligns with expected state transitions. Deviations from the normal state machine behaviour are treated as anomalies.

This approach allows the detector to capture protocol misuse, not just malformed packets.

</br>

Once anomalous behaviour is detected, the system moves to mitigation. The paper focuses on malicious or misbehaving xApps that generate non-compliant traffic toward the RIC or E2 nodes.

The anomaly traffic detector flags the xApp based on its traffic behaviour. After detection:

- Alerts are raised to the near-RT RIC
- The offending xApp can be throttled, isolated, or blocked

The key point here is that mitigation does not rely on knowing exactly what the xApp did wrong internally. It only depends on how the xApp behaves on the network, which makes the approach general and scalable.

<p align="center">
    <img src="../../NTN/notes-png/mitigation of malicious xApp non-compliant packet attacks using anomaly traffic detector.png" alt="Mitigation Of Malicious xApp Non-Compliant Packet Attacks Using Anomaly Traffic Detector" />
    <br />
    <em>Mitigation Of Malicious xApp Non-Compliant Packet Attacks Using Anomaly Traffic Detector</em>
</p>

This part shows how the detector handles packets with malformed or non-compliant fields. Even if such packets pass basic checks, their traffic patterns and field-level irregularities can still be detected as anomalies.

<p align="center">
    <img src="../../NTN/notes-png/malformed packet fields detection results.png" alt="Malformed Packet Fields Detection Results" />
    <br />
    <em>Malformed Packet Fields Detection Results</em>
</p>

The results demonstrate that the anomaly detector can:
- Identify malformed packet behaviour
- Distinguish it from normal traffic
- Trigger detection without deep packet inspection

The figure shows clear separation between normal and malicious traffic patterns, indicating that the detector can effectively flag malformed packet attacks during runtime.

## Validation & Evaluation & Results
This section evaluates whether the anomaly traffic detector actually works during runtime, and more importantly, whether it breaks near-real-time performance or not.

### Experimental Setup
- The detector is deployed inside the near-RT RIC.
- Traffic is generated to represent:
- Normal E2 communication
- Attack scenarios, especially DoS-style abnormal traffic
- Latency and traffic volume are measured with and without the detector enabled.

### Latency Impact

<p align="center">
    <img src="../../NTN/notes-png/Latency time with and without detector.png" alt="Latency Time With And Without Detector & E2 Traffic During And After DoS Attack" />
    <br />
    <em>Latency Time With And Without Detector & E2 Traffic During And After DoS Attack</em>
</p>

The first result looks at latency overhead introduced by the anomaly traffic detector.

- Without the detector, the near-RT RIC shows baseline latency under normal traffic.
- With the detector enabled, latency increases slightly due to traffic monitoring and analysis.
- However, the increase remains within acceptable near-real-time limits.

The key takeaway here is that:
- The detector does not significantly affect RIC responsiveness.
- Even under attack conditions, latency does not spike uncontrollably.

So basically, adding the detector doesn’t break the near-RT control loop.

### E2 Traffic Behaviour During DoS Attack
The second part of the figure shows E2 traffic volume over time during a DoS-style attack.
- During the attack phase, E2 traffic increases abnormally.
- The anomaly traffic detector identifies this deviation from normal traffic patterns.
- Once detection happens, mitigation actions are triggered.

After mitigation:
- E2 traffic levels drop back toward normal.
- The near-RT RIC regains stable operation.

This shows that the detector is not only detecting anomalies, but also enabling effective response to runtime attacks.

### Detection Effectiveness
From the results:
- Abnormal traffic patterns caused by DoS and malformed packet attacks are clearly distinguishable from normal traffic.
- The detector successfully flags these anomalies without relying on packet payload inspection. Detection works for both:
    - Malicious xApp-generated traffic
    - Attacks coming through the E2 interface

### 

## Key Concepts 
- Traffic-Behaviour-Based security for near-RT RIC
- Unified anomaly detection for xApps and E2 Interface
- State machine-based modelling of E2 communication
- Runtime detection without deep packet inspection
- Lightweight integration near-RT RIC

## Terminologies To Study
- DoS (Denial of Service)
    - Attacks that overwhelm the RIC with abnormal traffic.
    - Evaluated as one of the main attack scenarios.

- Anomaly Detection (ML-based)
    - Detects deviations from learned normal behaviour.
    - Used instead of signature-based detection.

## Standard References
- O-RAN Alliance Specs
- 3GPP RAN Architecture Specs
- E2 Protocols and Service Models
- Anomaly Detection in Network Security Literature

## Open Questions / Gaps
- How well would this anomaly detector work in a real operator network where traffic patterns change a lot (mobility spikes, flash crowds, failures)?
- The detector is behaviour-based — how do we tune thresholds to avoid false positives without missing real attacks?
- They test mainly DoS-style and malformed traffic. What about slow, stealthy attacks that gradually change behaviour?
- Since the near-RT RIC is assumed trusted, what happens if part of the RIC itself is compromised or misconfigured?
- Can anomaly detection alone be enough, or should it be combined with signature-based or rule-based methods?
- How scalable is this approach for large multi-cell deployments or thousands of UEs?
- In practice, what mitigation should be applied first once an anomaly is detected (block, throttle, alert only)?

## Cross-Referenced O-RAN Specifications
- O-RAN Architecture Description (WG1)
- O-RAN Near-RT RIC and E2 Interface Specification (WG3)
- O-RAN WG11 Security Architecture Specification
- O-RAN WG11 Threat Modeling and Risk Assessment
- O-RAN WG11 Security Test Specifications

Source: https://simewu.com/spec-reader/
