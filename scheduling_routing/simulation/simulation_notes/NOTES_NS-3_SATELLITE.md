...
- [Study Note 3: NS-3 Satellite Module](#study-note-3-ns-3-satellite-module)
  - [Table of Contents](#table-of-contents)
- [Directory Structure And Helper-Channel Interaction](#directory-structure-and-helper-channel-interaction)
  ...
  - [Example Simulation Workflow](#example-simulation-workflow)
    - [**Simple Simulation**](#simple-simulation)
    - [Simulation With: Configuration (Channel And Scenario), Nodes Creation, and many more](#simulation-with-configuration-channel-and-scenario-nodes-creation-and-many-more)
      - [Phase 1: Boilerplate and Configuration](#phase-1-boilerplate-and-configuration)
      - [Phase 2: Creating the Network Nodes](#phase-2-creating-the-network-nodes)
      - [Phase 3: Setting Up the Satellite Helper and Channel](#phase-3-setting-up-the-satellite-helper-and-channel)
      - [Phase 4: Installing Network Devices and the IP Stack](#phase-4-installing-network-devices-and-the-ip-stack)
      - [Phase 5: Assigning IP Addresses](#phase-5-assigning-ip-addresses)
      - [Phase 6: Running the Simulation](#phase-6-running-the-simulation)
      - [Summary](#summary)
  - [Satellite Module `satellite-helper.cc`, `satellite-channel-model.cc`](#satellite-module-satellite-helpercc-satellite-channel-modelcc)
    - [1. The Role of Helpers](#1-the-role-of-helpers)
      - [**a. What Are Helpers?**](#a-what-are-helpers)
    - [2. `SatHelper` — The Core Builder](#2-sathelper--the-core-builder)
      - [**Purpose**](#purpose)
      - [**Key Responsibilities**](#key-responsibilities)
      - [**Layer Connection Flow**](#layer-connection-flow)
      - [**Example Internal Flow (simplified)**](#example-internal-flow-simplified)
    - [3. `SatHelper` — The Core Builder](#3-sathelper--the-core-builder)
      - [**Location**](#location)
      - [**Purpose**](#purpose-1)
      - [**Key Components**](#key-components-1)
      - [**Propagation Flow**](#propagation-flow)
      - [4. Integration Between Helper and Model Layers](#4-integration-between-helper-and-model-layers)
      - [**Simplified Call Stack**](#simplified-call-stack)
- [Device \& PHY-MAC Exploration](#device--phy-mac-exploration)
  - [Analyze SatNetDevice](#analyze-satnetdevice)
    - [Core Responsibilities](#core-responsibilities)
    - [Key Data Members](#key-data-members)
    - [Installation Process via Helper](#installation-process-via-helper)
      - [**a. Entry Point**](#a-entry-point)
      - [**b. Configuration Steps**](#b-configuration-steps)
    - [Data Flow](#data-flow)
      - [**Sending Flow**](#sending-flow)
      - [**Receiving Flow**](#receiving-flow)
    - [Connection Flow Between Layers](#connection-flow-between-layers)
      - [**a. Internal Wiring**](#a-internal-wiring)
      - [**b. How Helpers Attach Components**](#b-how-helpers-attach-components)
  - [PHY and MAC Layer Analysis](#phy-and-mac-layer-analysis)
    - [File Overview](#file-overview)
    - [Data Flow Overview](#data-flow-overview)
    - [MAC Layer — `Transmit()` and `Receive()`](#mac-layer--transmit-and-receive)
      - [a. Transmit()](#a-transmit)
      - [b. Receive()](#b-receive)
    - [PHY Layer — `Transmit()` and `Receive()`](#phy-layer--transmit-and-receive)
      - [a. Transmit()](#a-transmit-1)
      - [b. Receive()](#b-receive-1)
    - [Channel Interaction — `SatChannel`](#channel-interaction--satchannel)
  - [Tracing and Statistics](#tracing-and-statistics)
    - [How Logs are Generated](#how-logs-are-generated)
    - [Functions Responsible for Writing Trace Data](#functions-responsible-for-writing-trace-data)
    - [Locating the Trace Connection Code](#locating-the-trace-connection-code)
    - [Trace \& Logging Hooks](#trace--logging-hooks)
    - [Relationships with Other Components](#relationships-with-other-components)
    - [Logging and Trace Output](#logging-and-trace-output)
    - [Summary of Interactions](#summary-of-interactions)
- [Tracing and Logging System](#tracing-and-logging-system)
  - [1. NS-3 Satellite Trace System](#1-ns-3-satellite-trace-system)
  - [`satellite-log.cc`](#satellite-logcc)
    - [Purpose](#purpose-2)
    - [Key Concept: Trace Registration with `AddTraceSource`](#key-concept-trace-registration-with-addtracesource)
  - [`satellite-packet-trace.cc`](#satellite-packet-tracecc)
    - [Purpose](#purpose-3)
    - [How the Trace Works](#how-the-trace-works)
  - [`m_packetTrace` and Related Callbacks](#m_packettrace-and-related-callbacks)
    - [What It Is](#what-it-is)
    - [How It Connects](#how-it-connects)
    - [The Flow in One Line:](#the-flow-in-one-line)
  - [Typical Example (Simplified)](#typical-example-simplified)
  - [Summary](#summary-1)
  - [2. NS-3 Event Trigger Analysis](#2-ns-3-event-trigger-analysis)
  - [Tracking Packet Event Emission (SND, RCV, ENQ, DRP)](#tracking-packet-event-emission-snd-rcv-enq-drp)
  - [How `SatNetDevice::Send()` and `Receive()` Link MAC and PHY Layers](#how-satnetdevicesend-and-receive-link-mac-and-phy-layers)
    - [a. `SatNetDevice::Send()`](#a-satnetdevicesend)
    - [b. `SatNetDevice::Receive()`](#b-satnetdevicereceive)
  - [Log Level Mapping (ND, LLC, MAC, PHY, CH)](#log-level-mapping-nd-llc-mac-phy-ch)
  - [Summary](#summary-2)
  - [3. NS-3 Practical Test \& Rerun](#3-ns-3-practical-test--rerun)
    - [Modify Trace Verbosity (Disable PHY Logs)](#modify-trace-verbosity-disable-phy-logs)
      - [Where to Edit](#where-to-edit)
    - [Option 1 — Disable PHY Traces in Code](#option-1--disable-phy-traces-in-code)
    - [Option 2 — Disable PHY Traces Using NS\_LOG Environment Variable](#option-2--disable-phy-traces-using-ns_log-environment-variable)
  - [Rerun the Example](#rerun-the-example)
  - [Compare Before \& After Logs](#compare-before--after-logs)
    - [Before (Full Verbosity)](#before-full-verbosity)
    - [After (PHY Disabled)](#after-phy-disabled)
  - [Analyze Changes](#analyze-changes)
  - [Optional: Restore or Fine-Tune Verbosity](#optional-restore-or-fine-tune-verbosity)
  - [Summary](#summary-3)
- [Channel \& Propagation Mastery](#channel--propagation-mastery)
    - [ns-3 Satellite Channel](#ns-3-satellite-channel)
  - [The Role of the Channel](#the-role-of-the-channel)
  - [1. `contrib/satellite/model/satellite-channel.cc`](#1-contribsatellitemodelsatellite-channelcc)
    - [How It Receives and Delivers a Signal](#how-it-receives-and-delivers-a-signal)
  - [1. `contrib/satellite/model/satellite-propagation-delay-model.cc`](#1-contribsatellitemodelsatellite-propagation-delay-modelcc)
    - [How It Calculates Delay](#how-it-calculates-delay)
  - [The Complete Workflow](#the-complete-workflow)
  - [ns-3 Satellite Propagation Delay \& Fading Models](#ns-3-satellite-propagation-delay--fading-models)
    - [1. Propagation Delay Computed from Satellite Geometry](#1-propagation-delay-computed-from-satellite-geometry)
      - [How it Works](#how-it-works)
  - [2. Fading Models and Their Pluggable Architecture](#2-fading-models-and-their-pluggable-architecture)
  - [3. Exploring Different Fading Models](#3-exploring-different-fading-models)
    - [How They Are "Plugged In"](#how-they-are-plugged-in)
    - [Common Satellite Fading Models](#common-satellite-fading-models)
  - [Tracing the ns-3 PHY-to-Channel Signal Flow](#tracing-the-ns-3-phy-to-channel-signal-flow)
    - [The Signal Flow: From Transmission to Reception](#the-signal-flow-from-transmission-to-reception)
      - [1. The Call to `SatPhy::Transmit()`](#1-the-call-to-satphytransmit)
      - [2. The Packet's Journey Through the Channel](#2-the-packets-journey-through-the-channel)
    - [Confirming Events with `PacketTrace.log`](#confirming-events-with-packettracelog)
      - [Analyzing the Log](#analyzing-the-log)
  - [Summary: ns-3 Satellite Channel Concepts](#summary-ns-3-satellite-channel-concepts)
    - [The Channel's Role](#the-channels-role)
    - [How Delay is Calculated](#how-delay-is-calculated)
    - [Fading and Signal Strength](#fading-and-signal-strength)
    - [Tracing the Signal Flow](#tracing-the-signal-flow)
- [Scenario \& Example Customization](#scenario--example-customization)
  - [ns-3 Satellite Example Scenarios: A Walkthrough](#ns-3-satellite-example-scenarios-a-walkthrough)
  - [1. Node Creation (`CreateNodes`)](#1-node-creation-createnodes)
    - [What it is:](#what-it-is-1)
    - [How it works:](#how-it-works-1)
    - [Positioning is Key](#positioning-is-key)
  - [2. Channel \& Device Configuration](#2-channel--device-configuration)
    - [What it is](#what-it-is-2)
    - [The `SatHelper`](#the-sathelper)
    - [How it works](#how-it-works-2)
  - [3. Application Setup (Generating Traffic)](#3-application-setup-generating-traffic)
    - [What it is](#what-it-is-3)
    - [How it works:](#how-it-works-3)
  - [4. Setting and Loading Parameters](#4-setting-and-loading-parameters)
    - [Data Rate](#data-rate)
    - [Frequency](#frequency)
    - [Delay (Important Distinction!)](#delay-important-distinction)
  - [Running Simulations and Observing Outputs](#running-simulations-and-observing-outputs)
  - [1. Running the Example Scenarios](#1-running-the-example-scenarios)
  - [2. Locating the Log Files](#2-locating-the-log-files)
    - [Expected Log Files By Example](#expected-log-files-by-example)
      - [A. `sat-tutorial-example`](#a-sat-tutorial-example)
      - [B. `sat-iot-example`](#b-sat-iot-example)
  - [Comparing `PacketTrace.log` Results](#comparing-packettracelog-results)
    - [How to compare Event Sequences](#how-to-compare-event-sequences)
  - [Trying Examples And Modify Parameters.](#trying-examples-and-modify-parameters)
    - [1. Trying Examples](#1-trying-examples)
    - [2. Locating The Example Dir Files](#2-locating-the-example-dir-files)
    - [3. Locate Log / Trace Outputs](#3-locate-log--trace-outputs)
    - [4. Compare Both Log File](#4-compare-both-log-file)
  - [Modify Something While Running Example](#modify-something-while-running-example)
    - [1. Choose a Simple but Visible Parameter](#1-choose-a-simple-but-visible-parameter)
    - [2. Locate and Open the Example File](#2-locate-and-open-the-example-file)
    - [3. Modify The Parameter](#3-modify-the-parameter)
    - [4. Rerun the Simulation](#4-rerun-the-simulation)
    - [5. Compare Logs](#5-compare-logs)
  - [6. Analyze The Differences](#6-analyze-the-differences)
    - [Interpretation](#interpretation)
- [Integrating Everything: End-to-End system understanding](#integrating-everything-end-to-end-system-understanding)
  - [The Packet Flow Path (End-to-End)](#the-packet-flow-path-end-to-end)
    - [1. Creation (at GT-A)](#1-creation-at-gt-a)
    - [2. Uplink Transmission (From GT-A)](#2-uplink-transmission-from-gt-a)
    - [3. Uplink Propagation (Space)](#3-uplink-propagation-space)
    - [4. Reception (At Satellite)](#4-reception-at-satellite)
    - [5. Downlink Transmission (from Satellite)](#5-downlink-transmission-from-satellite)
    - [6. Downlink Propagation (Space)](#6-downlink-propagation-space)
    - [7. Final Reception (at GT-B)](#7-final-reception-at-gt-b)
    - [8. Consumption (at GT-B)](#8-consumption-at-gt-b)
      - [This is a visual graph for explaining how the E2E satellite simulation:](#this-is-a-visual-graph-for-explaining-how-the-e2e-satellite-simulation)
    - [Where Logs are Triggered in the Flow](#where-logs-are-triggered-in-the-flow)
  - [Run and Observe Full Simulation](#run-and-observe-full-simulation)
    - [1. Execute simulation](#1-execute-simulation)
    - [2. Open Resulting Files](#2-open-resulting-files)
    - [3. Locate SND -\> RCV Pairs](#3-locate-snd---rcv-pairs)
      - [Step 1: Find a "SND" Event](#step-1-find-a-snd-event)
      - [Step 2: Find a "RCV" Event](#step-2-find-a-rcv-event)
      - [Step 3: Try to follow a single packet](#step-3-try-to-follow-a-single-packet)
        - [Lets break it down](#lets-break-it-down)
        - [What can we infer from the results](#what-can-we-infer-from-the-results)
        - [Example interpretation of 3 packets](#example-interpretation-of-3-packets)
  - [Code-Level Tracing](#code-level-tracing)
    - [1. `satellite-net-device.cc` (The Front Door)](#1-satellite-net-devicecc-the-front-door)
    - [2. `satellite-mac.cc` (The Traffic Manager)](#2-satellite-maccc-the-traffic-manager)
    - [3. `satellite-phy.cc` (The Radio)](#3-satellite-phycc-the-radio)
- [Experiment \& Report: Create my own minimal simulation](#experiment--report-create-my-own-minimal-simulation)
  - [Observe Output](#observe-output)
    - [Locate log files.](#locate-log-files)
    - [Use grep, head, and tail to see send/receive activity.](#use-grep-head-and-tail-to-see-sendreceive-activity)
      - [head](#head)
      - [tail](#tail)
    - [Confirm data is flowing end-to-end (GW → SAT → UT)](#confirm-data-is-flowing-end-to-end-gw--sat--ut)
  - [Analyze \& Visualize Results](#analyze--visualize-results)
    - [Summarize what happen in logs:](#summarize-what-happen-in-logs)
      - [When the packet was sent](#when-the-packet-was-sent)
      - [A simple diagram of the data path](#a-simple-diagram-of-the-data-path)
